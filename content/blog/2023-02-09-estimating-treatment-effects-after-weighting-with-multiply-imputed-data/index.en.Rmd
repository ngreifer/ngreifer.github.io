...
author: Noah Greifer
date: '2023-02-10'
slug: estimating-treatment-effects-after-weighting-with-multiply-imputed-data
categories:
  - R
tags:
  - propensity-scores
...
Although `{MatchThem}` has functionality for fitting models to the imputed datasets that incorporate the weights, for our purposes, it is better to extract the imputed datasets and fit each model manually in a loop. We'll use `glm()` to do so, though the `{MatchThem}` and `{WeightIt}` documentation may recommend `syrvey::svyglm()` because it correctly computes the robust standard errors. We'll do that later using `{marginaleffects}` and `{clarify}` functions so it's okay that we don't do it now. We'll use a quasi-binomial model because we have weights.
